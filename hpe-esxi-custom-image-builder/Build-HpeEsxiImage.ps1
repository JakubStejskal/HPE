...
if ($needBase) {
    ...
    Write-Host "   NOTE: this is the VMware OFFLINE BUNDLE '...-depot.zip' - NOT the installer .iso." -ForegroundColor DarkGray
    Write-Host "         (the bootable ISO is generated by this script from the bundle)" -ForegroundColor DarkGray
    $BaseDepot = Read-FilePath -Prompt 'Select the VMware ESXi BASE offline bundle   (VMware-ESXi-...-depot.zip)' `
                    -Filter 'ESXi base offline bundle (*-depot.zip;*.zip)|*-depot.zip;*.zip|All files (*.*)|*.*' -InitialDir $initDir
}
